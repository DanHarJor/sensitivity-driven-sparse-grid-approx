...
class SpectralScoresAllFiltered(DimensionAdaptivity):

    def __init__(self, dim, tols, tols_dims, init_multiindex, max_level, level_to_nodes, spectral_op_obj):
        
        self._dim                 = dim
        self._tols                 = tols
        self._tol_dims             = tols_dims
    ...
    def __get_activated_dir(self, multiindex):

        activated_dir = np.zeros(self._dim, dtype=int)

        for d in range(self._dim):
            if multiindex[d] > 1:
                activated_dir[d] = 1

        return activated_dir
    ...
    def do_one_adaption_step_preproc(self):
        ...
        max_i_total_local_var     = self.__spectral_op_obj.get_local_total_var_all(max_i)
        max_i_activated_dir     = np.zeros(self._dim, dtype=int)

        for d in range(self._dim):
            max_i_activated_dir[d] = max_i_total_local_var[d] >= self._tol_dims[d]

        print('multiindex with MAX SCORE', max_i)
        # print 'assciated local variances', self.__spectral_op_obj.get_local_var_level_active_set_all(self._multiindex_bin, self._multiindex_set, max_i)
        # print 'associated total local_variances', max_i_total_local_var
        # print 'acivated total local_variances', max_i_activated_dir
        ...
        if np.sum(max_i) == self._dim:
            neighbors_i = Multiindex(self._dim).get_successors(max_i)
        else:
            neighbors_i = Multiindex(self._dim).get_successors_with_history(max_i, self._multiindex_set)

        print('ITS ASSOCIATED NEIGHBORS')
        print(neighbors_i)

        for neighbor in neighbors_i:
            if self._is_O_admissible(neighbor):

                activated_dir = self.__get_activated_dir(neighbor)

                if np.max(neighbor) <= 2 or np.count_nonzero(activated_dir) == np.count_nonzero(max_i_activated_dir) == \
                                        np.count_nonzero(max_i_activated_dir * activated_dir):
                    print('ADDED SUPER admissible neighbor', neighbor, activated_dir)
                    ...
                    print(neighbor, 'DOES NOT SATISFY COND')
                    self._H[self._key_H] = neighbor
                    self._key_H         += 1
            else:

                print(neighbor, 'IS NOT ADMISSIBLE')    

                self._H[self._key_H] = neighbor
                self._key_H         += 1

        print('HISTORY')
        print(list(self._H.values()))

        # for neighbor_history in self._H.values():
        #     if self._is_O_admissible(neighbor_history):

        #         print 'ADMISSIBLE NEIGH FROM HISTORY', neighbor_history

                # activated_dir = self.__get_activated_dir(neighbor_history)

                # if np.count_nonzero(activated_dir) == np.count_nonzero(max_i_activated_dir) == \
                #                         np.count_nonzero(max_i_activated_dir * activated_dir):
                #     print 'ADDED SUPER admissible neighbor from HISTORY', neighbor_history, activated_dir
                
                #     local_multiindices.append(neighbor_history)

                #     self._key_A         += 1
                #     self._A[self._key_A] = neighbor_history

                #     self._multiindex_set.append(neighbor_history)

                #     local_basis_neighbor = np.array([self._get_no_1D_grid_points(n) - 1 for n in neighbor_history], dtype=int)
                #     self._update_local_basis(neighbor_history.tolist(), local_basis_neighbor)

                #     key_to_del = self.__find_keys(self._H, neighbor_history)

                #     del self._H[key_to_del]
